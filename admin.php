...
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col antialiased">
                ...
                <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Gerencie referências olfativas, estoque e faça upload de imagens do seu computador</p>
                        </div>
                        <button id="seedLocalDbBtn" class="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold text-xs rounded-xl transition-all border border-rose-200 flex items-center gap-2">
                            <i data-lucide="refresh-cw" class="w-3.5 h-3.5"></i>
                            <span>Restaurar 5 Produtos com Referências</span>
                        </button>
                    </div>
    ...
    <div id="editProductModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <p class="text-xs text-slate-500 mb-6">Atualize preço, referência olfativa, quantidade em estoque e foto do computador.</p>
            ...
            <form id="editProductForm" class="space-y-4 text-xs">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Referência Olfativa (Inspirado em / Contratipo)</label>
                        <input type="text" id="editProductOlfactoryRef" placeholder="Ex: Miss Dior, 212 VIP Black..." required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:border-rose-500 font-medium text-rose-700">
                    </div>
                    <div>
    ...
    <script>
        ...
        document.getElementById('editProductForm').addEventListener('submit', async (e) => {
            ...
            if (result.status === 'success') {
                closeEditModal();
                alert('Produto, referência olfativa e quantidade em estoque salvos com sucesso!');
                fetchLocalData();
                loadCrmStats();
            } else {
