...
<body class="bg-brand-softBg text-neutral-800 font-sans antialiased selection:bg-brand-rose selection:text-white">
        ...
        <div class="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="checkoutFullForm" class="space-y-6 text-xs">
                ...
                <div class="p-4 rounded-2xl bg-rose-50 border border-rose-200 space-y-3">
                    <span class="font-bold text-rose-900 block text-xs">4. Cupom de Desconto</span>
                    <div class="flex gap-2">
                        <input type="text" id="chkCoupon" placeholder="Insira seu código" class="w-full px-3.5 py-2.5 rounded-xl border border-rose-300 focus:outline-none focus:border-rose-500 font-mono text-sm uppercase">
                        <button type="button" onclick="applyCoupon()" class="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-semibold shrink-0 text-sm">Aplicar</button>
                    </div>
                    <div id="couponMessage" class="text-xs font-semibold hidden"></div>
                </div>
    ...
    <a href="mailto:contato@uselovely.com.br" class="fixed bottom-6 right-6 z-40 bg-neutral-800 hover:bg-neutral-900 text-white p-3.5 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center gap-2 group" title="Falar por E-mail">
        <i data-lucide="mail" class="w-6 h-6"></i>
        <span class="max-w-0 overflow-hidden whitespace-nowrap group-hover:max-w-xs transition-all duration-500 ease-in-out text-xs font-bold pl-1">E-mail de Contato</span>
    </a>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-neutral-800">
                ...
                <div class="space-y-3">
                    ...
                    <ul class="space-y-2 text-xs text-neutral-400 font-light">
                        <li>
                            <a href="mailto:contato@uselovely.com.br" class="hover:text-white transition-colors text-rose-300 font-semibold flex items-center gap-1.5">
                                <i data-lucide="mail" class="w-4 h-4"></i> contato@uselovely.com.br
                            </a>
                        </li>
    ...
    <script>
        ...
        window.applyCoupon = async function() {
            const codeInput = document.getElementById('chkCoupon');
            const code = codeInput.value.trim().toUpperCase();
            const msgEl = document.getElementById('couponMessage');
            
            if (!code) {
                msgEl.textContent = 'Digite um código de cupom.';
                msgEl.className = 'text-xs font-semibold text-rose-600 block mt-2';
                return;
            }
            
            msgEl.textContent = 'Validando cupom...';
            msgEl.className = 'text-xs font-semibold text-neutral-500 block mt-2';
            ...
            try {
                ...
                if (data.status === 'success') {
                    const c = data.coupon;
                    msgEl.textContent = `Cupom aplicado! ${c.type === 'percentage' ? c.value + '% de desconto.' : 'R$ ' + c.value + ' de desconto.'}`;
                    msgEl.className = 'text-xs font-semibold text-emerald-600 block mt-2';
                    codeInput.disabled = true;
                    // Note: Actual calculation happens on the backend, 
                    // but we could visually update a total here if we want.
                } else {
                    msgEl.textContent = data.message || 'Cupom inválido.';
                    msgEl.className = 'text-xs font-semibold text-rose-600 block mt-2';
                }
            } catch (err) {
                msgEl.textContent = 'Erro ao validar o cupom.';
                msgEl.className = 'text-xs font-semibold text-rose-600 block mt-2';
            }
        };
        ...
        document.getElementById('checkoutFullForm').addEventListener('submit', async (e) => {
            ...
            const checkoutPayload = {
                ...
                couponCode: document.getElementById('chkCoupon').value.trim(),
                items: cart
            };
